"""Turns a saved recording (test case JSON) into a standalone, readable
Playwright script under generated_scripts/. A dev can open it and see
exactly what will get clicked/double-clicked/right-clicked/filled/selected/
submitted/scrolled and in what order, or run it by hand outside the
dashboard.

This is also the ONLY replay implementation in the project - executor/
run_execution.py does not re-implement resolve_and_act itself, it runs
this generated script as a subprocess and reads back the structured
result the script writes out. That keeps "what the generated script
does" and "what /api/test/run actually executes" the same code, always.
"""
import json
import logging
import os
...
# scripts generated from an edited recording go here instead, so they never
# mix with (or overwrite the name of) a script generated from the original
EDITED_OUTPUT_DIR = OUTPUT_DIR / "edited"
EDITED_OUTPUT_DIR.mkdir(exist_ok=True)

# string.Template ($name) instead of str.format() on purpose - the generated
# code is full of literal {} (dicts, f-strings) that would need constant
# brace-escaping with .format(); $ doesn't collide with anything in Python
SCRIPT_TEMPLATE = Template('''"""
Auto-generated from recording: $source_name
Generated at: $generated_at
Original recorded (production) URL: $start_url
...
Run directly with: python $script_filename [qa_url] [output_json_path] [screenshot_dir] [headless] [product_name]
...
def generate_script(test_case: dict, out_name: str = None, output_dir: Path = None) -> Path:
    ...
    for action in test_case.get("actions", []):
        steps.append({
            "action_type": action.get("action_type"),
            "value": action.get("value"),
            "locator_profile": action.get("locator_profile") or {},
            "bounding_box": action.get("bounding_box") or {},
            "click_strategy": action.get("click_strategy"),
            # opt-in per-action override for wait_for_stable_state() -
            # absent/false on nearly every recorded action (the automatic
            # inference already covers the common case), only ever set by
            # hand-editing a recording's JSON for a step whose target is
            # known to depend on a state transition the automatic
            # inference wouldn't otherwise catch
            "wait_for_stable_state": action.get("wait_for_stable_state"),
            "wait_for_stable_state_timeout_ms": action.get("wait_for_stable_state_timeout_ms"),
            # fields for the opt-in "add_to_cart_and_verify" action_type -
            # None on every ordinary action, only ever populated on a
            # step explicitly authored/edited to use it
            "click_selector": action.get("click_selector"),
            "confirmation_selector": action.get("confirmation_selector"),
            "confirmation_text": action.get("confirmation_text"),
            "next_action_selector": action.get("next_action_selector"),
            "next_action_text": action.get("next_action_text"),
            "verification_selector": action.get("verification_selector"),
            "verification_text": action.get("verification_text"),
            "max_retries": action.get("max_retries"),
            "confirmation_timeout_ms": action.get("confirmation_timeout_ms"),
            "verification_timeout_ms": action.get("verification_timeout_ms"),
            ...
            "delay_before_ms": action.get("delay_before_ms"),
            "new_tab": action.get("new_tab"),
            "page_id": action.get("page_id", 0),
            "from_page_id": action.get("from_page_id"),
            "to_page_id": action.get("to_page_id"),
            "from_url": action.get("from_url"),
            "to_url": action.get("to_url"),
            "remaining_page_id": action.get("remaining_page_id"),
            "check_mode": action.get("check_mode"),
            "target": action.get("target"),
            "label": action.get("label"),
            "take_screenshot": action.get("take_screenshot"),
            "screenshot": action.get("screenshot"),
            "screenshot_path": action.get("screenshot_path"),
            "capture_screenshot": action.get("capture_screenshot"),
            "save_screenshot": action.get("save_screenshot"),
            "file_path": action.get("file_path"),
            "target_path": action.get("target_path"),
            "capture_as": action.get("capture_as"),
            "compare_to": action.get("compare_to"),
            "count_as": action.get("count_as"),
            "expected_count": action.get("expected_count"),
            "count_a": action.get("count_a"),
            "count_b": action.get("count_b"),
            "operator": action.get("operator"),
            "labels": action.get("labels"),
            "check": action.get("check"),
            "expected_state": action.get("expected_state"),
            "min_value": action.get("min_value"),
            "max_value": action.get("max_value"),
            "detect_by": action.get("detect_by"),
            # validate_text/validate_attribute/validate_value's own fields
            "attribute_name": action.get("attribute_name"),
            "match_mode": action.get("match_mode"),
            # optional, human-typed override (see the Recording Editor's
            # own rename button) - absent on nearly every action, in
            # which case _derive_action_name() computes a readable name
            # from the fields above instead
            "name": action.get("name"),
        })

    name = out_name or f"{test_case.get('name', 'test')}_script.py"
    if not name.endswith(".py"):
        name += ".py"

    target_dir = output_dir or OUTPUT_DIR
    # EDITED_OUTPUT_DIR (and any subfolder of it) means this script was
    # generated from an edited recording, not the original capture -
    ...
    # the generated script with a NameError the moment it ran.
    # RECORDED viewport (see record_session.py's Recorder.stop() and the
    # new_context() call in run() below for the full story) - an old
    # recording made before this field existed just doesn't have
    # viewport_width/height at all, so these fall back to Playwright's
    # own no-viewport-specified default (1280x720), never to
    # REPLAY_VIEWPORT_HEIGHT_PX's old fixed 900 - matching what that old
    # recording's OWN preceding scroll-target math almost certainly
    # already assumed, back when this feature didn't exist yet.
    ...
    script_text = SCRIPT_TEMPLATE.substitute(
        ...
        steps_literal=pformat(steps, indent=4, width=100, sort_dicts=False),
    )
    ...
    recorded_count = len(test_case.get("actions", []))
    generated_count = len(steps)
    if generated_count != recorded_count:
        # steps is built with exactly one entry per action above, so this
        # can only happen if that construction is changed later without
        # keeping the 1:1 mapping - catching it here means a broken
        # generator fails loudly instead of silently producing a
        # truncated script that could still be marked PASS at replay
        logger.error(
            "generated script action count mismatch for %s: recorded=%d generated=%d",
            name, recorded_count, generated_count,
        )
        print(
            f"WARNING: generated script action count ({generated_count}) "
            f"does not match recorded/JSON action count ({recorded_count})"
        )

    logger.info("generated script at %s (%d steps)", path, len(steps))
    return path
