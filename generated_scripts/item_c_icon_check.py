...
Generated at: 2026-09-21T08:45:50.460926
...
Run directly with: python item_c_icon_check.py [qa_url] [output_json_path] [screenshot_dir] [headless] [product_name]
...
_ICON_FONT_PUA_RE = re.compile(r"[-]")
...
def _strip_icon_font_text(text):
    ...
    Returns the ORIGINAL text unchanged if it looks like normal text
    (even mixed with the occasional icon glyph inline), or "" if it's
    predominantly icon-font PUA characters, signaling the caller to fall
    back to a better field (aria-label/title/nearby label/icon class
    hint) instead. Never raises; None/non-string input returns "".
    """
    if not isinstance(text, str) or not text.strip():
        return text or ""
    stripped = text.strip()
    pua_chars = _ICON_FONT_PUA_RE.findall(stripped)
    if len(pua_chars) >= max(1, len(stripped) - 2):
        return ""
    return text
...
def resolve_and_act(page, step, prev_action_type=None, fast_fail=False, turbo=False):
    ...
    def _attempt_tiers():
        ...
        def _post_click_check(el_for_fp):
            ...
            if not post_ok and dom_changed:
                print(
